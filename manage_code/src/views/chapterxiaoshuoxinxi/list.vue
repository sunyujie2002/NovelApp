	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form">
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.chaptertitle" placeholder="章节标题"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button v-if="$toolUtil.storageGet('role') !== '管理员'" type="success"
						@click="addClick">新增</el-button>
					<el-button type="info" :disabled="selRows.length == 1 ? false : true"
						@click="infoClick(null)">详情</el-button>
					<el-button v-if="$toolUtil.storageGet('role') !== '管理员'" type="primary"
						:disabled="selRows.length == 1 ? false : true" @click="editClick">修改</el-button>
			...
			<el-table v-loading="listLoading" border :stripe='false' @selection-change="handleSelectionChange"
				...
				<el-table-column prop="chapternum" :resizable='true' :sortable='true' align="left" header-align="left"
					label="章节数">
					<template #default="scope">
						{{ scope.row.chapternum }}
					</template>
				</el-table-column>
				<el-table-column prop="chaptertitle" :resizable='true' :sortable='true' align="left" header-align="left"
					label="章节标题">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left"
					header-align="left">
					<template #default="scope">
						<el-button type="info" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
...
//基础信息
onMounted(() => {
	context?.$toolUtil.storageSet("chapterRefid", route.query.refid);
})
//列表数据
const list = ref(null)
...
const getList = () => {
	...
	if (searchQuery.value.chaptertitle && searchQuery.value.chaptertitle != '') {
		params['chaptertitle'] = '%' + searchQuery.value.chaptertitle + '%'
	}
	context?.$http({
		url: `${tableName}/page`,
		method: 'get',
