	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('discusszuojia', '新增')">新增</el-button>
					<el-button type="primary" :disabled="selRows.length == 1 ? false : true" @click="editClick"
						v-if="btnAuth('discusszuojia', '修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length ? false : true"
						@click="delClick(null)">删除</el-button>
				</div>
			</div>
			<br>
			<el-table v-loading="listLoading" border :stripe='false' @selection-change="handleSelectionChange"
				ref="table" :data="list" @row-click="listChange">
				<el-table-column :resizable='true' align="left" header-align="left" type="selection" width="55" />
				<el-table-column label="序号" width="70" :resizable='true' :sortable='true' align="left"
					header-align="left">
					<template #default="scope">{{ scope.$index + 1 }}</template>
				</el-table-column>
				<el-table-column prop="nickname" :resizable='true' :sortable='true' align="left" header-align="left"
					label="用户名">
					<template #default="scope">
						{{ scope.row.nickname }}
					</template>
				</el-table-column>
				<el-table-column prop="content" :resizable='true' :sortable='true' align="left" header-align="left"
					label="评论内容">
					<template #default="scope">
						{{ scope.row.content }}
					</template>
				</el-table-column>
				<el-table-column prop="reply" :resizable='true' :sortable='true' align="left" header-align="left"
					label="回复内容">
					<template #default="scope">
						{{ scope.row.reply }}
					</template>
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left"
					header-align="left">
					<template #default="scope">
						<el-button v-if="btnAuth('discusszuojia', '查看评论')" type="warning"
							@click="commentClick(scope.row.id)">查看评论</el-button>
						<el-button type="warning" v-if="$toolUtil.storageGet('role') !== '管理员'"
							@click="replyClick(scope.row.id)">回复</el-button>
					</template>
				</el-table-column>
			</el-table>
...
//基础信息
const tableName = 'discusszuojia'
const formName = '作家评论表'
...
const commentClick = (id) => {
	context?.$router.push('/discussdiscusszuojia?refid=' + id)
}
