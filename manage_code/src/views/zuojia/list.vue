	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form">
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.zuojiazhanghao" placeholder="作家账号"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('zuojia', '新增')">新增</el-button>
					<el-button v-if="btnAuth('zuojia', '查看')" type="info" :disabled="selRows.length == 1 ? false : true"
						@click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length == 1 ? false : true" @click="editClick"
						v-if="btnAuth('zuojia', '修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length ? false : true" @click="delClick(null)"
						v-if="btnAuth('zuojia', '删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table v-loading="listLoading" border :stripe='false' @selection-change="handleSelectionChange"
				ref="table" v-if="btnAuth('zuojia', '查看')" :data="list" @row-click="listChange">
				...
				<el-table-column label="照片" width="120" :resizable='true' :sortable='true' align="left"
					header-align="left">
					<template #default="scope">
				...
				<el-table-column prop="xingbie" :resizable='true' :sortable='true' align="left" header-align="left"
					label="性别">
					<template #default="scope">
						{{ scope.row.xingbie }}
					</template>
				</el-table-column>
				<el-table-column prop="dianhua" :resizable='true' :sortable='true' align="left" header-align="left"
					label="电话">
					<template #default="scope">
						{{ scope.row.dianhua }}
					</template>
				</el-table-column>
				<el-table-column prop="zuojiajianjie" :resizable='true' :sortable='true' align="left"
					header-align="left" label="作家简介">
					<template #default="scope">
						{{ scope.row.zuojiajianjie }}
					</template>
				</el-table-column>
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left"
					header-align="left">
					<template #default="scope">
						<el-button type="info" v-if="btnAuth('zuojia', '查看')"
							@click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('zuojia', '查看评论')" type="warning"
...
const getList = () => {
	...
	params['sort'] = 'id'
	params['order'] = 'desc'
	if (searchQuery.value.zuojiazhanghao && searchQuery.value.zuojiazhanghao != '') {
		params['zuojiazhanghao'] = '%' + searchQuery.value.zuojiazhanghao + '%'
	}
...
const commentClick = (id) => {
	context?.$router.push('/discusszuojia?refid=' + id)
}
//初始化
const init = () => {
	getList()
