			...
			<div class="info_view">
				<div class="title_view">
					<div class="detail_title">
						{{ detail.zuojiaxingming }}
					</div>
					<div class="follow" v-if="!collectType" @click="collectClick(41)">
					...
					<div class="follow" v-if="collectType" @click="collectClick(-1)">
						...
						<span class="textActive">取消关注</span>
					</div>
				</div>
				<div class="info_item">
				...
				<div class="info_item">
					<div class="info_label">作家简介</div>
					<div class="info_text">{{ detail.zuojiajianjie }}</div>
				</div>
				<!-- <div class="btn_view">
					<el-button v-if="centerType && (detail.ispay == '未支付' || !detail.ispay) && btnFrontAuth('zuojia', '支付')"
						class="approval_btn" @click="payClick">支付</el-button>
					<el-button class="edit_btn" v-if="centerType && btnAuth('zuojia', '修改')" type="primary"
						@click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType && btnAuth('zuojia', '删除')" type="danger"
...
const getDetail = () => {
	context?.$http({
		...
		title.value = res.data.data.zuojiaxingming
...
const collectClick = (type) => {
	if (type == 41 && !collectType.value) {
		...
		context?.$http({
			...
			collectType.value = true
			getCollect()
			context?.$toolUtil.message('关注成功', 'success')
		})
	}
