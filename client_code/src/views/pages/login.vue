	...
	<div>
		<div class="login_view">
			<el-form :model="loginForm" class="login_form">
				<div class="title_view">小说阅读网登录界面</div>
				<div class="index" @click="handleIndex">进入首页</div>
				...
				<div class="list_type" v-if="userList.length > 1">
					...
					<el-select v-model="loginForm.role" placeholder="请选择用户类型">
						<el-option v-for="(item, index) in userList" :label="item.roleName"
				...
				<div class="btn_view">
					...
					<el-button class="register" type="primary" @click="handleRegister('duzhe')">注册读者</el-button>
				</div>
			</el-form>
		</div>
	</div>
...
const rememberPassword = context?.$toolUtil.storageGet('isRememberPassword') ? ref(Boolean(context?.$toolUtil.storageGet('isRememberPassword'))) : ref(true)
//注册
...
const login = () => {
	context?.$http({
		...
		context?.$toolUtil.storageSet("isRememberPassword", rememberPassword.value);
