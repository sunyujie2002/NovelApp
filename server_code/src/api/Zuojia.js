...
import ZuojiaModel from '../models/ZuojiaModel'
...
export default ({ config, db }) => {
	...
	api.all('/login', async (req, res) => {
		...
		try {
			...
			let userinfo = await ZuojiaModel.findOne({ where: { zuojiazhanghao: req.query.username, mima: pwd } })
			if (userinfo === null) {
			...
			const token = jwt.sign(
				{
					...
					username: userinfo.dataValues.zuojiazhanghao,
					role: '管理员',
                    loginUserColumn: 'zuojiazhanghao',
                    tableName: 'zuojia'
				},
				config.jwtSecret,
				{
			...
			userinfo.dataValues.loginUserColumn = 'zuojiazhanghao'
			userinfo.dataValues.tableName = 'zuojia'
	...
	api.post('/register', async (req, res) => {
		...
		try {
			...
			const userinfo = await ZuojiaModel.create(req.body)
	...
	api.all('/session', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await ZuojiaModel.findOne({ where: { id: req.session.userinfo === undefined ? jwt.decode(req.headers.token).id : req.session.userinfo.id } }))
		} catch(err) {
	...
	api.all('/resetPass', async (req, res) => {
    	...
    	let pwd = '123456'
			...
			await ZuojiaModel.update({
				mima: pwd
			}, {
				where: {
				  zuojiazhanghao: req.query.username
				}
			})
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let zuojiajianjie = req.query.zuojiajianjie
			if (zuojiajianjie) {

				if (zuojiajianjie.indexOf('%') != -1) {
					where.zuojiajianjie = {
						[Op.like]: zuojiajianjie
					}
				} else {
					where.zuojiajianjie = {
						[Op.eq]: zuojiajianjie
			...
			let result = await ZuojiaModel.findAndCountAll({
				order: [[sort, order]],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await ZuojiaModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await ZuojiaModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM zuojia WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM zuojia WHERE " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM zuojia WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM zuojia WHERE " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM zuojia WHERE " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM zuojia WHERE " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "zuojia"
			let where = " WHERE 1 = 1 "
			let tableName = req.session.userinfo === undefined ? jwt.decode(req.headers.token).tableName : req.session.userinfo.tableName
			if(tableName == 'zuojia') {
				where += " AND zuojiazhanghao = '" + jwt.decode(req.headers.token).username + "' ";
			}
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM zuojia " + where + " GROUP BY " + columnName + " LIMIT 10" 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM zuojia " + where + " GROUP BY " + xColumnName + " DESC LIMIT 10"
